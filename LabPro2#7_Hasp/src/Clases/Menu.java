...
public class Menu extends JFrame {
    ...
    private JPanel crearFormularioTrofeo() {
        ...
        gbc.gridy = 0;
        gbc.weightx = 0.3;
        formPanel.add(crearEtiquetaUI("Username:", Color.WHITE), gbc);
        gbc.gridx = 1;
        gbc.gridy = 0;
        gbc.weightx = 0.7;
        formPanel.add(txtUsernameTrophy, gbc);

        gbc.gridx = 0;
        gbc.gridy = 1;
        formPanel.add(crearEtiquetaUI("Tipo Trofeo:", Color.WHITE), gbc);
        gbc.gridx = 1;
        gbc.gridy = 1;
        formPanel.add(cmbTrofeoTipo, gbc);

        gbc.gridx = 0;
        gbc.gridy = 2;
        formPanel.add(crearEtiquetaUI("Juego/Descripción:", Color.WHITE), gbc);
        gbc.gridx = 1;
        gbc.gridy = 2;
        formPanel.add(txtJuegoDescripcion, gbc);

        gbc.gridx = 0;
        gbc.gridy = 3;
        formPanel.add(crearEtiquetaUI("Ruta Imagen:", Color.WHITE), gbc);
    ...
    private void limpiarCampos() {
        ...
        if (txtUsernameTrophy != null) {
            txtUsernameTrophy.setText("");
        }
        if (txtJuegoDescripcion != null) {
            txtJuegoDescripcion.setText("");
        }
    ...
    private void probarAgregarUsuario(String username) {
        if (username.isEmpty()) {
            showCustomDialog("El campo de Username no puede estar vacio.",
        ...
        try {
            ...
            showCustomDialog("Usuario agregado correctamente.",
                    "Operación Exitosa", JOptionPane.INFORMATION_MESSAGE);
        } catch (IOException e) {
            showCustomDialog("Error al agregar usuario.",
    ...
    private void probarDesactivarUsuario(String username) {
        ...
        try {
            ...
            showCustomDialog("Usuario desactivado correctamente.",
                    "Excelente Proceso", JOptionPane.INFORMATION_MESSAGE);
        } catch (IOException e) {
            showCustomDialog("Error al desactivar usuario.",
                    "Error", JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    private void probarMostrarInfo(String username) {
        ...
        if (username.isEmpty()) {
            showCustomDialog("El campo Username está vacío",
                    "Error", JOptionPane.ERROR_MESSAGE);
            return;
        }
        ...
        try {
            ...
            showCustomDialog("Error al mostrar información",
                    "Error", JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    private void probarAgregarTrofeo() {
        ...
        if (username.isEmpty() || juegoDesc.isEmpty()) {
            showCustomDialog("Faltan datos (Username o Juego).",
                    "Error de Entrada", JOptionPane.ERROR_MESSAGE);
            return;
        }
...
<<<<<<< HEAD:LabPro2#7_Hasp/src/Clases/Menu.java
            ...
            psnUsers.addTrophieTo(username, juegoDesc, juegoDesc, tipo);

            lblResultadoTrophy.setIcon(null);
            lblResultadoTrophy.setText("<html><center> TROFEO AGREGADO<br>Usuario: <b>"
                    + username + "</b><br>Tipo: <b>"
                    + tipo + "</b><br>Juego: <b>"
...
>>>>>>> 3ceeb6b6f438736e0f12cd7aa0b78deec73d23c7:LabPro2#7_Hasp/src/labpro2/pkg7_hasp/Menu.java
            ...
            showCustomDialog("Trofeo agregado correctamente.",
                    "Operación Exitosa", JOptionPane.INFORMATION_MESSAGE);

        } catch (Exception e) {
            showCustomDialog("Error al agregar trofeo: " + e.getMessage(),
                    "Error", JOptionPane.ERROR_MESSAGE);
        }
    }
}
